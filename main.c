        ...
        if ( !pause )
        {
            ...
            for ( int i = 0; i < activeEnemies; i++ )
            {
                if ( enemy[ i ].active )
                {
                    ...
                    {
                        enemy[ i ].rec.x =
                            GetRandomValue( screenWidth, screenWidth + 1000 );
                        enemy[ i ].rec.y = GetRandomValue(
                            0, screenHeight - enemy[ i ].rec.height );
                    }
                }
            }
            ...
            for ( int i = 0; i < NUM_SHOOTS; i++ )
            {
                if ( shoot[ i ].active )
                {
                    ...
                    for ( int j = 0; j < activeEnemies; j++ )
                    {
                        if ( enemy[ j ].active )
                        {
                            if ( CheckCollisionRecs( shoot[ i ].rec,
                                                     enemy[ j ].rec ) )
                            {
                                shoot[ i ].active = false;
                                enemy[ j ].rec.x  = GetRandomValue(
                                    screenWidth, screenWidth + 1000 );
                                enemy[ j ].rec.y = GetRandomValue(
                                    0, screenHeight - enemy[ j ].rec.height );
                                shootRate = 0;
                                enemiesKill++;
                                score += 100;
                            }
...
void DrawGame( void )
{
    ...
    if ( !gameOver )
    {
        ...
        // Draw Variables
        TraceLog( LOG_INFO, "----------------------------------------" );
        TraceLog( LOG_INFO, "ShootRate:     %d", shootRate );
        TraceLog( LOG_INFO, "Alpha:         %f", alpha );
        TraceLog( LOG_INFO, "activeEnemies: %d", activeEnemies );
        TraceLog( LOG_INFO, "enemiesKill:   %d", enemiesKill );
        TraceLog( LOG_INFO, "smooth:        %d", smooth );
        TraceLog( LOG_INFO, "++++++++++++++++++++++++++++++++++++++++" );
